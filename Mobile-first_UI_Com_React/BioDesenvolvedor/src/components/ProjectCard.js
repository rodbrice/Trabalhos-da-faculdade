...
const ProjectCard = ({ project, onPress }) => {
  ...
  return (
    <TouchableOpacity style={[styles.card, isDarkMode && styles.cardDark]} onPress={onPress} activeOpacity={0.7}>
      <Text style={[styles.name, isDarkMode && styles.textDark]}>{project.name}</Text>
      <Text style={[styles.desc, isDarkMode && styles.descDark]} numberOfLines={2}>{project.description}</Text>
      <View style={styles.footer}>
        ...
        <View style={styles.stats}>
          <Text style={[styles.stat, isDarkMode && styles.statDark]}>⭐ {project.stars}</Text>
          <Text style={[styles.stat, isDarkMode && styles.statDark]}>🔱 {project.forks}</Text>
        </View>
      </View>
    </TouchableOpacity>
  );
...
const styles = StyleSheet.create({
  ...
  name: { fontSize: 18, fontWeight: '600', color: colors.light.text, marginBottom: 8 },
  ...
  lang: { backgroundColor: colors.primary, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12 },
  langText: { color: '#ffffff', fontSize: 12, fontWeight: '600' },
  stats: { flexDirection: 'row', gap: 12 },
  stat: { fontSize: 14, color: colors.light.textSecondary },
  statDark: { color: colors.dark.textSecondary },
});
