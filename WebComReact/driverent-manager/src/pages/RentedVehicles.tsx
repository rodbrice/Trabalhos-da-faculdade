...
// Página de listagem de veículos alugados
...
const RentedVehicles: React.FC = () => {
  const navigate = useNavigate();
  const { veiculos, onEdit, onDelete } = useOutletContext<OutletContext>();
  
  const alugadosVehicles = veiculos.filter(v => v.status === 'alugado');
  ...
  if (alugadosVehicles.length === 0) {
    return (
      <div className="no-vehicles">
        <p>😔 Nenhum veículo alugado no momento.</p>
      </div>
    );
  }
        ...
        <tbody>
          {alugadosVehicles.map((veiculo) => (
            <tr key={veiculo.id}>
              ...
              <td>
                <span className="status-badge alugado">
                  🚗 {veiculo.status}
                </span>
              </td>
